// Initialize localStorage if it's not present
...
function addExpense() {
  ...
  if (paidBy && amount && date && usage) {
    expenses.push({ id: Date.now(), paidBy, amount, date, usage });
...
function editExpense(id) {
  ...
  if (expense) {
    ...
    // Remove the original expense from the list (Optional step, can be skipped)
...
window.onload = function () {
  initializeData(); // Initialize data from localStorage
